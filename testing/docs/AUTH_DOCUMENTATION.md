...
2.  **تسجيل الدخول (Login)**:
    *   يرسل المستخدم طلب `POST` إلى `/api/v1/auth/login` مع `loginIdentifier` (البريد الإلكتروني أو رقم الهاتف) و `password`.
    *   يتم التحقق من وجود المستخدم في قاعدة البيانات عبر البريد الإلكتروني أو رقم الهاتف.
    *   يتم مقارنة كلمة المرور المقدمة مع كلمة المرور المشفرة المخزنة باستخدام `bcrypt.compare`.
    *   إذا كانت بيانات الاعتماد صحيحة، يتم إنشاء JWT يحتوي على `id`, `roleId`, و `name` للمستخدم.
