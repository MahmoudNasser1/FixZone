...
const BreadcrumbLink = React.forwardRef(({ 
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "transition-colors hover:text-foreground text-blue-600 hover:text-blue-800",
        className
      )}
      {...props}
    />
...
const EnhancedBreadcrumb = React.forwardRef(({ 
  ...
  const displayItems = React.useMemo(() => {
    ...
    return [firstItem, { type: 'ellipsis' }, ...lastItems];
  }, [items, maxItems]);
      ...
      <BreadcrumbList>
        ...
        {displayItems.map((item, index) => (
          <React.Fragment key={index}>
            ...
            {index < displayItems.length - 1 && item.type !== 'ellipsis' && (
              <BreadcrumbSeparator />
            )}
          </React.Fragment>
        ))}
