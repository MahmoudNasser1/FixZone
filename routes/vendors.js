...
router.delete('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const [result] = await db.query('UPDATE Vendor SET deletedAt = CURRENT_TIMESTAMP WHERE id = ? AND deletedAt IS NULL', [id]);
    if (result.affectedRows === 0) {
      return res.status(404).send('Vendor not found or already deleted');
    }
    res.json({ message: 'Vendor deleted successfully (soft delete)' });
  } catch (err) {
    console.error(`Error soft deleting vendor with ID ${id}:`, err);
    res.status(500).send('Server Error');
  }
});
